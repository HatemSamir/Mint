var util = {
  ...
  register: function () {
    ...
    for (i = 0; i < children.length; i += 1) {
      child = children[i]
      child.addEventListener("click", formSelect.f.select.toggle)
    }
  },
  unregister: function () {
      ...
      if (util.f.isElem(child)) {
        if (child.classList.contains("deselect")) {
          ...
          if (!field.dataset.value) {
            util.f.addStyle(child, ["FontSize"], ["16px"])
          }
          ...
          for (ii = 0; ii < nested_children.length; ii += 1) {
            ...
            if (util.f.isElem(nested_child) && nested_child.classList.contains("selected")) {
              ...
              setTimeout(function (c, v) {
                c.innerHTML = v
                util.f.addStyle(c, ["Opacity", "Transform", "TransitionDuration"], [1, "translateY(0px)", ".0s"])
              }, time, nested_child, value)
            }
          }
        } else if (child.tagName == "SPAN") {
          util.f.addStyle(child, ["FontSize", "Top", "color"], [".8rem", "-19px", "#fcb316"])
         }
       }
    }
  },
  toggle: function (event) {
    ...
    switch (true) {
      ...
      case (child.classList.contains("option")):
        ...
        formSelect.f.select.selection(child, parent)
        break
    }
    parent.classList.contains("focused") ? formSelect.f.select.blur(parent) : formSelect.f.select.focus(parent)
  }
  }
  }}
  window.onload = formSelect  .f.init.register
